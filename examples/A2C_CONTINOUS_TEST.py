...
from Configurations import A2CInvertedDoublePendulumConfig
from Agents import A2C
from Trainer import Trainer
import random

def test_policy(env, agent, num_episodes=5, normalization_weights=np.array([]), video=True, max_steps=2048):
    if video:
        env = RecordVideo(env, 'RL-Grimoire/videos', episode_trigger=lambda e: True)
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # Hyperparams
    config = A2CInvertedDoublePendulumConfig()
    ...
    if not test:
        ...
        agent = A2C(observation_space=observation_space, action_space = action_space, hyperparams=config.agent_params, device=device)
