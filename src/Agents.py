...
class PPO2(Agent):
    ...
    def learnV2(self, batch: list[Transition], num_envs: int, batch_size: int, num_rounds: int = 8, mini_batch_size: int = 32) -> dict[str, Tensor]:
        self.update_count += 1
        ...
        for _ in range(num_rounds):
            ...
            with torch.no_grad():
                ...
                old_values_flat = old_values.view(-1) if self.hyperparams.value_loss_clipping else None
                prev_log_probs_flat = prev_log_probs.view(-1)
            ...
            for start_idx in mini_batch_start_indices:
                ...
                with torch.no_grad():
                    mb_old_values = old_values_flat[ids] if self.hyperparams.value_loss_clipping else None
                    mb_states = states_flat[ids]
                    mb_actions = actions_flat[ids]
                    mb_old_log_probs = prev_log_probs_flat[ids]
                    mb_advantages = advantages_all[ids]
                    mb_targets = torch.unsqueeze(targets_flat[ids], dim=-1)
                ...
                # prev_log_probs_flat[ids] = log_probs # TODO Test this
...
class PPO2Recurrent(PPO2):
    def __init__(
        ...
        if self.is_continous():
            ...
            self.actor = GaussianGradientPolicyRecurrent(
                self.state_size, 
                self.num_actions, 
                self.hidden_size,
                device=device
            ...
            self.critic = ValueNetworkRecurrent(
                self.state_size, 
                self.hidden_size, 
                device
            )
            ...
            if self.hyperparams.value_loss_clipping:
                self.old_critic = ValueNetworkRecurrent(
                    self.state_size, 
                    self.hidden_size, 
                    device
                )
    ...
    def get_actions(self, state: torch.Tensor, dones: torch.Tensor, eval=False)->tuple[Tensor, Tensor]:
        if self.is_continous():
            if self.hyperparams.recurrent_policy:
                mean, std = self.actor(state, dones)
                normal = torch.distributions.Normal(mean, std) 
    ...
    def learn(self, batch: list[Transition], num_envs: int, batch_size: int, num_rounds: int = 12, mini_batch_size: int = 512) -> dict[str, Tensor]:
        ...
        with torch.no_grad():
            ...
            dones_shifted = torch.cat([dones_shifted, torch.full((dones.shape[0], 1), False, dtype=torch.float32, device=dones.device)], dim=-1)

            values = self.critic(states, dones)
            next_values = self.critic(next_states, dones_shifted)
            old_values = self.old_critic(next_states, dones) if self.hyperparams.value_loss_clipping else None
        ...
        for _ in range(num_rounds):
            ...
            # Generate a random order of mini-batches
            mini_batch_start_indices = list(range(0, states.size(0), mini_batch_size))
            # np.random.shuffle(mini_batch_start_indices) # TODO test shuffling and not shuffling 
            
            # Process each mini-batch
            for start_idx in mini_batch_start_indices:

                self.actor.transformer.reset_memory()
                ...
                # Compute policy distribution parameters
                loc, scale = self.actor(mb_states, mb_dones)
                dist = Normal(loc, scale)
                log_probs = dist.log_prob(mb_actions).sum(dim=-1)
