...
def convert_brax_to_gym(name: str, frame_stack: bool = True, stack_size: int = 4, **kwargs):
    device = torch.device(
...
class BraxFrameStack(gym.Wrapper):
    def __init__(self, env: gym.Env, stack_size: int, device: torch.device = torch.device("cpu")):
        ...
        self.stacks = torch.zeros((self.num_envs, self.stack_size, self.env.observation_space.shape[1]), dtype=torch.float32).to(self.device)
    ...
    def step(self, action):
        ...
        # Shift old observations up in the stack and add new observations
        self.stacks = torch.cat((self.stacks[:, 1:], obs.unsqueeze(1)), dim=1)
    ...
    def get_obs(self):
        # Convert the tensor to be of shape (num_envs, stack_size * state_size)
        obs = self.stacks.view(self.num_envs, -1)
